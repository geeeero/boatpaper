...
The choice of prior distribution is often debatable, especially 
when prior knowledge is limited or data are scarce.
Robust Bayesian analysis accounts for this issue by
inquiring whether posterior inferences change substantially
when the prior distribution is varied within a set of `reasonable' prior distributions.
The imprecise probability approach is similar but formalises that sets of probability distributions
should be taken to accurately model prior knowledge.
...
conflict sensitivity, allows to reflect \emph{strong prior-data agreement} by increased posterior precision.
...
Prior information is encoded by a \emph{prior distribution} $f(\vartheta)$,
expressing the expert's opinion on $\vartheta$.
...
\end{align}
The posterior distribution underlies all inferences.
%or the \emph{posterior predictive distribution},
%giving the distribution of further observations based on the posterior.
%
The choice of prior distribution to encode expert knowledge is often debatable.
...
The method presented in this paper also uses sets of priors, but with different interpretation in line with
theory of imprecise probability \cite{itip,1991:walley}, considering sets of posterior distributions as the 
proper re to express the precision of probability statements themselves:
...
The approach has been applied in common-cause failure modelling \cite{Troffaes2014a}
...
updating is straightforward if one uses \emph{conjugate} prior distributions, for which
...
\includegraphics[width=\textwidth]{boatshape-posterior-mik.pdf}%
%}
...
\includegraphics[width=\textwidth]{boatshape-posterior-normal.pdf}%
%}
...
\begin{figure}
\includegraphics[width=\textwidth]{pppboat-ipmu1.pdf}
...
\begin{figure}
\includegraphics[width=\textwidth]{pppboat-ipmu2.pdf}
...
The framework opens a wide field of research on set shapes for catering to other objectives in inference.
