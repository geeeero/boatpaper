...
\begin{abstract}
we will propose a novel idea for generating parameter prior sets in a conjugate setting %$\PZ$
...
***imprecision to account for model uncertainty, Generalized Bayes' Rule, ***
...
The choices for $\PZ$ discussed in the literature are:
\begin{enumerate}[(a)]
...
and was later generalised by \cite{Walter2009a}
to sample distributions of the canonical exponential form \eqref{eq:expofam-sampledens}.%
\footnote{\label{foot:luckmodels}%
%from \cite[\S 4.4]{itip-statinf}:\\
More precisely, \cite[see Section~\ref{sec:jstp}]{Walter2009a} proposed
...
which were already used in \cite{Walter2007a}
...
but nevertheless follow the same update step.} %This is elaborated in Section~\ref{sec:jstp} \cite{Walter2009a} 
...
type~(\ref{enum:rectangular}) allows for \pdc\ sensitivity.

The model we propose belongs to type~(\ref{enum:generalset}), and results in models that give more precise inferences when prior and data are in agreement.
