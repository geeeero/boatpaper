...
There is a clear relation between prior and posterior imprecision of inferences
through the magnitude of $\MZ$ and $\MN$, which in turn is governed by the size of $\PZ$ and $\PN$, respectively.
Obviously, the smaller $\YZ$, c.p.\ the smaller $\PN$ and thus the more precise the inferences based on $\PN$.
Furthermore, the larger $n$ relative to (values in the range of) $\nz$,
c.p.\ the smaller $\PN$, such that imprecision generally decreases with sample size.
...
as the size of $\PN$ does not vary with $\ttau(x)$ \cite[Eq. (3.4), p.~64]{diss-gw}.
...
such that prior information (encoded in $\yz$) and data (via the sufficient statistic $\ttau(x)$) can have a range of weights,
resulting in the stretch of $\yn$ values in $\PN$ being the larger the further $\ttau(x)$ is away from $\YZ$
...
For further details on this framework, see \citet[\S 3.1]{diss-gw}.
...
the prior expectation of the mean sample statistic $\ttau(x)$ (here, $s/n$),
%$\E\big[\E[\ttau(x) \mid \psi] \mid \nz, \yz\big]$,
$\eta_1$ is only slightly more difficult to interpret.
From \eqref{eq:trafotony} we can derive that coordinates $(\eta_0,\eta_1) \in \Eta$ satifying
\begin{align}
