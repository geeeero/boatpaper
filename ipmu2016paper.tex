...
\end{align*}
We call the coordinates $\arg\min_{(\eta_0,\eta_1) \in \EN} \yn$ and $\arg\max_{(\eta_0,\eta_1) \in \EN} \yn$
...
It is curved along its length in the direction of its constant rays of expectation,
this leads to smaller $\Delta_y(\PN)$ as compared to a rectangular $\PZ$ with the same prior range $\Delta_y(\PZ)$,
as illustrated in Figure~\ref{fig:boatshape-posterior-normal}.

The strong prior-data agreement effect is further enhanced
due to the touchpoints determining $\ynl$ and $\ynu$
being attained at higher values of $\eta_0$ for larger $n$,
as will be demonstrated in Section~\ref{***}.
...
at the boundary of the posterior expectation interval $[\ynl,\ynu]$ will be lower.
As an example, credible intervals for these `critical' distributions will be shorter, 
such that also unions of credible intervals,
...
and we discuss a number of technical results for this shape in Sections~\ref{sec:touchpoints} -- \ref{sec:generalupdate}.
...
will be discussed in Section~\ref{sec:boatshape-outlook}.****
...
\end{align}
A prior boatshape set with $\ezl=1$, $\ezu=6$, $a=2$, and $b=0.8$,
...
The parameters for the prior set are $\ezl=1$, $\ezu=6$, $a=2$, and $b=0.8$.
...
for the prior set and the posterior set resulting for data $\frac{s}{n}=\frac{4}{8}$,
...
The rectangular prior set with the same prior range for $\yz$ as the prior boatshape set
and the corresponding posterior sets are shown as dashed lines. 
Note that posterior boatshape sets are generally smaller than posterior rectangle sets.}
...
touchpoints $(\eta_0^l,\czl(\eta_0^l))$ and $(\eta_0^u,\czu(\eta_0^u))$.
This can be done by identifying the rays of constant expectation \eqref{eq:raysofconstantexpectation}
that are tangents to $\EZ$ and then solve for $\eta_0$.
With $\EZ$ being symmetric to the $\eta_0$ axis, we have $\eta_0^u = \eta_0^l$,
we will determine $\eta_0^u$ only by considering the upper contour tangent.
We get
\begin{align}
a - a \big(1 + b(\eta_0^u + 2)\big) e^{-b(\eta_0^u - \ezl)} &\stackrel{!}{=} 0
& &\Longleftrightarrow &
1 + b(\eta_0^u + 2) &\stackrel{!}{=} e^{b(\eta_0^u - \ezl)}\,.
\label{eq:eta0u}
\end{align}
This equation has only one solution for $\eta_0^u > \ezl$
that is, however, not available in closed form.
Generally, the nearer $\eta_0^u$ is to $\ezl$, the larger $\frac{d}{d\eta_0} \czu(\eta_0^u)$,
such that $\yzu$ %the upper expected value for the prior set
is further away from $\frac{1}{2}$.
Here, this means that the larger $\eta_0^u$, the more imprecise the prior parameter set.
