...
\begin{abstract}
Bayesian inference enables combination of observations with %(possibly subjective)
...
%(its derivation is described in detail in \cite{2015:mik-isipta}):
%\begin{equation}
\begin{align}
\label{eq:trafotony}
%\begin{aligned}
...
Denoting the bounds of this shadow by\vspace*{-1ex}
\begin{align*}
...
%Prior and posterior boatshape sets (for different observations) %with corresponding posterior sets for different observations
...
%}
%\caption[Boatshape prior set in the parametrization via $(\eta_0,\eta_1)$ and via $(\nz,\yz)$.]%
%{Boatshape prior set in the parametrization via $(\eta_0,\eta_1)$ (left) and via $(\nz,\yz)$ (right),
...
The touchpoint movement to the right means that the upper posterior touchpoint $\ezun$
...
that has a purely data-dependent translation as update step as well \cite[p.~56]{2015:mik-isipta}.
