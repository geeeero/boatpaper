...
when the success probability in each trial is $p$.
%
...
\label{eq:betadensny}
\end{align}
written here in terms of the \emph{canonical}%
...
so $\yz$ is is the prior expectation for $p$,
...
by upper index~${}\un$.
The posterior given that $s$ out of $n$ components function is a Beta distribution with updated parameters
\begin{align}
\nn &= \nz + n\,, &
\yn &= \frac{\nz}{\nz + n} \cdot \yz + \frac{n}{\nz + n} \cdot \frac{s}{n}\,.
\label{eq:nyupdate}
...
%The parametrisation in terms of prior mean and prior strength (or pseudocount)
