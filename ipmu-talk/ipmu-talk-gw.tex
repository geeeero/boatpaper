...
\begin{frame}{New Parameter Set Shape}
\begin{tikzpicture}
\uncover<1->{\node at (0,0) {\includegraphics[width=0.9\textwidth]{figs/boatshape-alone.pdf}};}
\end{tikzpicture}
\uncover<2->{%
\begin{align*}
\EZ = \{(\ezg,\eor) \colon \ezl \le \eta_0 \le \ezu, \czl(\eta_0) \le  \eta_1 \le \czu(\eta_0) \}\,,
\text{ where}
\end{align*}
\begin{align*}
...
\begin{frame}{New Parameter Set Shape: Update}
\begin{tikzpicture}
...
\uncover<2->{%
\node[fill=white] at (-3.3,1.5) {$\frac{s}{n}=\frac{4}{8}$};
\node[fill=white] at ( 2.0,1.5) {$\frac{s}{n}=\frac{8}{8}$};
}
\end{tikzpicture}
\end{frame}
...
\textbf{Outlook:}
\begin{itemize}
\item[\play] ***here for Beta-Binomial, but can be constructed for all distributions from exponential family
\end{itemize}}
\end{frame}
